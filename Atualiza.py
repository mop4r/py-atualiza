...
class App(QMainWindow):
    ...
    def finalize_update(self, files_copied, files_failed, success):
        ...
        if success and files_successfully_copied:
            ...
            if reply == QMessageBox.Yes:
                self.show_files_copied(files_successfully_copied)
        elif success:
            QMessageBox.information(self, 'Atualizado', 'Todos os arquivos já estão atualizados!')
        else:
            QMessageBox.critical(self, 'Erro', 'Houve um erro durante a atualização.')

    def show_files_copied(self, files):
        list_win = QMainWindow(self)
        list_win.setWindowTitle("Arquivos Transferidos")
